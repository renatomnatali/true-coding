...
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    ...
    const result = await recoverDevelopmentRun(id, runId)
    ...
    if (message === 'RUN_NOT_RECOVERABLE') {
      return NextResponse.json(
        { error: 'RUN_NOT_RECOVERABLE', message: 'A execução atual não pode ser retomada por este endpoint.' },
        { status: 409 }
      )
    }
